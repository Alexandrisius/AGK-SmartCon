...
namespace PipeConnect
{
    class SlopePipeFiting
    {
        public static void SlopePipe(Document doc, Connector con1, int typeSlope, double slope)
        {
            ...
            if (slope != 0 && mf!=null && mf.PartType == PartType.Elbow) 
            {
                using (var tx = new Transaction(doc))
                {
                    ...
                    if (typeSlope == 0)
                    {
                        ...
                        {
                            ElementTransformUtils.RotateElement(doc, con1.Owner.Id, lineZ, -Math.Abs(angle_fact - angle_desi));
                            angle_fact = con2.CoordinateSystem.BasisZ.AngleTo(XYZ.BasisZ);
                            count++;
                        }
                        ElementTransformUtils.RotateElement(doc, con1.Owner.Id, lineZ, angle_corr);// подкрутка на нужный угол
